...
- V1 supports polygon, text-mask, and svg-mask shapes
- Product framing should describe those as geometry input, value-derived input, and SVG mask input when talking about the user-facing feature set
- V1 is browser-first
...
- Keep `svg-mask` browser-first and path-based in v1; do not promise arbitrary raw SVG markup or external asset fetching
...
- Keep cheap pure-logic tests near consumer-example helpers when they validate reusable time/render preparation without standing up the full browser screen
